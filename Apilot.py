...
class Apilot(Plugin):
    ...
    def on_handle_context(self, e_context: EventContext):
        ...
        for domain in video_platforms.keys():
            if domain in content:
                if not self.alapi_token:
                    reply = self.create_reply(ReplyType.TEXT, "请先配置alapi的token")
                else:
                    video_url = self.parse_video_url(content)
                    if video_url.startswith("http"):
                        # 下载视频
                        video_path = self.download_video(video_url)
                        if video_path:
                            reply = self.create_reply(ReplyType.VIDEO, video_path)
                        else:
                            reply = self.create_reply(ReplyType.TEXT, "视频下载失败")
                    else:
                        reply = self.create_reply(ReplyType.TEXT, video_url)
    ...
    def get_help_text(self, verbose=False, **kwargs):
        ...
        if verbose:
            help_text += "\n📹 短视频解析：\n"
            help_text += "  发送短视频分享链接即可解析下载\n"
            help_text += "  支持平台：抖音、西瓜视频、快手、皮皮虾\n"
    ...
    def parse_video_url(self, share_url):
        """解析短视频链接"""
        url = BASE_URL_ALAPI + "video/url"
        payload = f"token={self.alapi_token}&url={share_url}"
        headers = {'Content-Type': "application/x-www-form-urlencoded"}
        ...
        try:
            ...
            if isinstance(response, dict) and response.get('code') == 200:
                ...
                return f"视频解析失败：{response.get('msg', '未知错误')}"
        except Exception as e:
            return self.handle_error(e, "视频解析失败")
    ...
    def cleanup_videos(self):
        ...
        try:
            ...
            for filename in os.listdir(self.video_dir):
                ...
                # 检查文件是否超过24小时
                if os.path.getmtime(file_path) < current_time - 86400:  # 86400 = 24小时
                    os.remove(file_path)
                    logger.info(f"已删除过期视频：{filename}")
        except Exception as e:
    ...
    def start_cleanup_schedule(self):
        """启动定时清理任务"""
        def run_schedule():
            schedule.every().day.at("00:00").do(self.cleanup_videos)
            while True:
